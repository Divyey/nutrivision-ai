...
- `GET /health` is API + database + registered services (auth, users, meals). Food readiness is `GET /api/v1/food/health` and is not on the aggregator.
...
- Meal diary is `MealService` → `/api/v1/meals/*`. Predict JSON still has no kcal. `dish_nutrition` is filled from `backend/data/dish_nutrition.csv` via the upsert script (Alembic creates empty tables only). Logging a `class_id` with no row returns 400.
