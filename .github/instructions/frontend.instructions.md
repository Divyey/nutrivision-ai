...
- `services/{Name}Service/{Name}Service.ts` — API clients. Today: `AuthService`, `UserService`, `DetectionService` (food predict client), `MealService` (diary), `NutritionService` (catalog search). Do not rename `DetectionService` to `FoodService` unless the ticket asks; the backend class is already `FoodService`.
...
- `types/` — DTOs that match backend JSON (`auth.ts`, `user.ts`, `detection.ts`, `meals.ts`, `nutrition.ts`). Keep them in sync when the API changes.
